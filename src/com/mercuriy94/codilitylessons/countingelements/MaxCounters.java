...
public class MaxCounters {
    ...
    public static void main(String[] args) {
        int[] testData1 = {3, 4, 4, 6, 1, 4, 4};
        System.out.println(Arrays.toString(solution(5, testData1)));
    }
    ...
    public static int[] solution(int N, int[] A) {
        ...
        for (int i = 0, count = A.length; i < count; i++) {
            ...
            if (value <= N) {
                int counter = result[value - 1];
                counter = Math.max(min + 1, counter + 1);
                maxCounter = Math.max(maxCounter, counter);
                result[value - 1] = counter;
            } else {
                min = maxCounter;
            }
